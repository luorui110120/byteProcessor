...
def Write64bitFloat(field_number, value, output):
    ...
    if(sys.version > '3'):
        ...
        bytesStr = bytesToHexString(bytesStr)
    else:
...
def ReEncode(messages, output):
    ...
    if sys.version > '3':
        ...
        keys = messages.iterkeys()
    for key in sorted(keys, key= lambda x: int(x.split(':')[1])):
        keyList = key.split(':')
        ...
        if wire_type == 'Varint':
            ...
            if sys.version > '3':
                bytesStr = []
                for elem in messages[key].encode('utf-8'):
                    bytesStr.append(elem)
            else:
...
def ParseProtoBufToJson(inbytes):
    messages = {}
    data = inbytes
    size = len(inbytes)
    ParseData(data, 0, size, messages)
    ...
    if sys.version > '3':
        outbuf = json.dumps(messages, indent=4, sort_keys=True, ensure_ascii=False)
    else:
        outbuf = json.dumps(messages, indent=4, sort_keys=True, ensure_ascii=False, encoding='utf-8')
    #print(outbuf)
    return outbuf;

def ParseJsonToProtoBuf(jsonData):
    if sys.version > '3':
        messages = json.loads(jsonData)
    else:
        messages = json.loads(jsonData, encoding='utf-8')
    output = list()
    ReEncode(messages, output)
    return bytearray(output)
    #return ''.join(['%02X' % b for b in byteout])
def main():
    if sys.argv[1] == "dec":
